...

{
    ...
    // Save the background color at the text position
    int backgroundColor = getpixel(fixed_x[node] + 6, fixed_y[node] - 40);

    // Draw a filled rectangle to cover the text and remove it
    setfillstyle(SOLID_FILL, backgroundColor);
    bar(fixed_x[node] + 6, fixed_y[node] - 40, fixed_x[node] + 6 + textwidth(const_cast<char*>(label.c_str())), fixed_y[node]);
